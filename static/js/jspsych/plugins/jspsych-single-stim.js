...
(function($) {
	jsPsych["single-stim"] = (function() {
		...
		plugin.create = function(params) {
			...
			params = jsPsych.pluginAPI.enforceArray(params, ['stimuli', 'choices',"prompt"]);
			...
			for (var i = 0; i < trials.length; i++) {
				...
				trials[i].prompt = (typeof params.prompt[i] === 'undefined') ? "" : params.prompt[i];
				trials[i].warning = (typeof params.warning === 'undefined') ? "" : params.warning;
			}
			return trials;
		};
		...
		plugin.trial = function(display_element, trial) {
			...
			if (trial.timing_stim > 0) {
				var t1 = setTimeout(function() {
					$('#jspsych-single-stim-stimulus').replaceWith('<div id="jspsych-VBET-warning"> '+trial.warning+'</div>');
